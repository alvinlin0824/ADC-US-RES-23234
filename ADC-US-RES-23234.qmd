...
#           lag_diff = KRSEQ01 - lag(KRSEQ01),
kgriv12 |>
   mutate(RoC = KRSEQ01 - lag(KRSEQ01) / as.numeric(difftime(`Ketone Date Time`,lag(`Ketone Date Time`),units = "hours")),
          mean_roc = mean(RoC,na.rm = T),
          sd_roc = sd(RoC,na.rm = T),
          Z_score = (RoC - mean_roc)/sd_roc,
          Z_score_status = case_when(abs(Z_score) >= 3 ~ "Flag",
                                     .default = ""),
          IQR = quantile(RoC,probs = 0.75,na.rm = T) - quantile(RoC,probs = 0.25,na.rm = T),
          IQR_status = case_when(RoC <= quantile(RoC,probs = 0.25,na.rm = T) - 1.5 * IQR |
                                 RoC >= quantile(RoC,probs = 0.75,na.rm = T) + 1.5 * IQR
                                   ~ "Flag",
                                     .default = ""),
          `Ketone Date Time` = as.character(`Ketone Date Time`),
          .by = c(Subject,`Ref Type`)) |> 
   select(!c(KRSEQ03,`Glucose Date Time`)) |> 
