...
#| fig-cap: "Histogram of Rate Deviation"
Ap_rate |>
   filter(!is.na(`Rate Deviation`)) |>
   ggplot() +
...
```{r eval = params$analysis}
#| label: System Agreement group by Reference type
#| column: page
#| tbl-cap: "System Agreement group by Reference Type"
Ap |> 
   system_agreement_fun(wider_name = NULL,group_var = c(`Ref Type`,Level)) |> 
...
```{r eval = params$analysis}
#| label: Bias plot by Reference
#| fig-align: center
Ap |> 
    ggplot(aes(x = KRSEQ01, y = `Difference(mmol/L)`, color = Site, shape = Site)) +
    geom_point() +
    facet_grid(cols = vars(`Ref Type`)) +
    labs(title = "Bias Plot", x = "Reference (mmol/L)", y = "Bias (mmol/L)") +
    theme_bw() +
    theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),legend.position = "bottom",plot.title = element_text(hjust = 0.5),legend.text = element_text(size = 8))
```
