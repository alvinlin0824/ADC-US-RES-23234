...
#| eval: false
# eTime = as.numeric(difftime(`Ketone Date Time`,lag(`Ketone Date Time`),units = "hours")),
...
kgriv12 |>
     mutate(iv_diff_pre1 = as.numeric(difftime(lag(`Ketone Date Time`, n = 1),`Ketone Date Time`,units = "hours")),
            ...
            # 
            # ketone_rate_pre3 = case_when(-30 <= lag(iv_diff_pre1, n = 2) & lag(iv_diff_pre1, n = 2) < 0 ~ 
            #  (lag(KRSEQ01, n = 3) - lag(KRSEQ01, n = 2)) / as.numeric(difftime(lag(`Ketone Date Time`, n = 3),lag(`Ketone Date Time`, n = 2),units = "hours")),
            #  .default = NA),
            # 
            # ketone_rate_pre4 = case_when(-30 <= lag(iv_diff_pre1, n = 3) & lag(iv_diff_pre1, n = 3) < 0 ~ 
            #  (lag(KRSEQ01, n = 4)-lag(KRSEQ01, n = 3)) / as.numeric(difftime(lag(`Ketone Date Time`, n = 4),lag(`Ketone Date Time`, n = 3),units = "hours")),
            #  .default = NA),
            # 
            # ketone_rate_pre5 = case_when(-30 <= lag(iv_diff_pre1, n = 4) & lag(iv_diff_pre1, n = 4) < 0 ~ 
            #  (lag(KRSEQ01, n = 5)-lag(KRSEQ01, n = 4)) / as.numeric(difftime(lag(`Ketone Date Time`, n = 5),lag(`Ketone Date Time`, n = 4),units = "hours")),
            #  .default = NA),
            ...
            ketone_rate_post1 = case_when(0 < iv_diff_post1 & iv_diff_post1 <= 30 ~ 
             ...
             .default = NA),
            # a =  lag(ketone_rate_post1, n = 1),
            # b = lead(ketone_rate_pre1,n = 1),
            # ketone_rate_pre2 = case_when(-30 <= lag(iv_diff_pre1, n = 1) & lag(iv_diff_pre1, n = 1) < 0 ~
            #  (lag(KRSEQ01, n = 2)-lag(KRSEQ01, n = 1)) / as.numeric(difftime(lag(`Ketone Date Time`, n = 2),lag(`Ketone Date Time`, n = 1),units = "hours")),
            #  .default = NA),
            # ketone_rate_post2 = case_when(0 < iv_diff_post1 & iv_diff_post1 <= 30 ~
            #              (lead(KRSEQ01, n = 2) - lead(KRSEQ01, n = 1)) / as.numeric(difftime(lead(`Ketone Date Time`, n = 2),lead(`Ketone Date Time`, n = 1),units = "hours")),
            #  .default = NA),
            .by = c(Subject,`Ref Type`)) |> 
    # mutate(self_Exculsion = case_when(
    #                               # Case I
    #                              round_normal(abs(ketone_rate_pre1),1) > 2.5 &
    #                              round_normal(abs(ketone_rate_post1),1) > 2.5 ~ "Flag",
    #                              (round_normal(abs(ketone_rate_pre1),1) > 2.5 & 
    #                              round_normal(abs(ketone_rate_pre1),1) <= 1) |
    #                               (round_normal(abs(ketone_rate_post1),1) > 2.5 & 
    #                              round_normal(abs(ketone_rate_post2),1) <= 1)  ~ "Flag",
    #                              
    #                              .default = "")
      
      
          #  self_Exculsion = case_when(# Case I
          #                        round_normal(abs(ketone_rate_pre1),1) > 2.5 &
          #                        round_normal(abs(ketone_rate_post1),1) > 2.5 ~ "Flag",
          #                        .default = ""),
          # # a =  lag(ketone_rate_post1, n = 1),
          # lag_Exculsion = case_when(round_normal(abs(ketone_rate_pre1),1) > 2.5
          #                            ~ "Flag",
          #                            .default = ""
          #                            ),
          # # b = lead(ketone_rate_pre1,n = 1),
          #  lead_Exculsion = case_when(round_normal(abs(ketone_rate_post1),1) > 2.5
          #                            ~ "Flag",
          #                            .default = ""
          #                            ),
           # a = lead(lag_Exculsion, n = 1),
           # b = lag(lead_Exculsion, n = 1)
    ...
    ungroup() |>
    mutate(avg = round_normal(avg_pre1_post1,1),
           Exclusion = case_when(
    ...
    # filter(me > 3) |> View()
    # # Exclusion
    # mutate(Exclusion = case_when(ketone_rate_pre1 > 0.75 & ketone_rate_pre4 > 0.75 & !is.na(ketone_rate_pre2) & !is.na(ketone_rate_pre3) & (ketone_rate_pre2 < -2 | ketone_rate_pre3 < -2) ~ "Rapid Ketone rate of change",
    #       ketone_rate_pre1 < -3 & is.na(ketone_rate_post1) ~ "Ketone ROC <-3 mg/dL/min and no subsequent ROC available",
    #          str_detect(IVVAL01,regex("Invalid",ignore_case = T)) ~ IVVAL01,
    #         .default = "")) |>
    # mutate(Exclusion = case_when(
    #           lead(Exclusion, n = 1) == "Rapid Ketone rate of change" & ketone_rate_pre1 < -2
    #            ~ "Ketone ROC<-2 mg/dL/min while the previous and subsequent ROC >0.75 mg/dL/min",
    #           lead(Exclusion, n = 2) == "Rapid Ketone rate of change" & ketone_rate_pre1 < -2
    #            ~ "Ketone ROC<-2 mg/dL/min while the previous and subsequent ROC >0.75 mg/dL/min",
    #           lead(Exclusion, n = 3) == "Rapid Ketone rate of change" & ketone_rate_pre1 < -2
    #            ~ "Ketone ROC<-2 mg/dL/min while the previous and subsequent ROC >0.75 mg/dL/min",
    #            Exclusion == "Rapid Ketone rate of change" ~ "",
    #           .default = Exclusion),.by = c(Subject,`Ref Type`)) |> View()
   # filter(Subject %in% c(unique(kgriv12$Subject)[seq(21,25,1)])) |> 
   # # filter(Subject %in% c("90006","90002","90003","90004","90005")) |> 
   # ggplot() +
   # aes(y = Subject, x = RoC) + 
   # ggridges::geom_density_ridges(quantile_lines = TRUE)
   # vroom_write("randox_quality.csv",delim = ",")
```
