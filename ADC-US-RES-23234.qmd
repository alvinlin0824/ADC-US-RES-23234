...
data <- data.frame(
  ...
  temperature = runif(100, min = 0, max = 10),
  price = runif(100, min = 0, max = 100)
)

# Plot with primary and secondary y-axes
ggplot(data, aes(x = day)) +
  geom_line(aes(y = temperature), color = "blue") +
  geom_line(aes(y = price / 10), color = "red") +  # Adjust the scale for the secondary axis
  scale_y_continuous(
    name = "Temperature (°C)",
...
dm |> 
   ...
   select(Subject,SEX:RACE, `Weight(Pound)` = VSORES03,`Weight(Kg)`:`Height(Meter)`) |> 
