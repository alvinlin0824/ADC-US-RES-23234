...
#   filter(!str_detect(DVYN,regex("Check here if no data recorded",ignore_case = T))) |> View()
#   transmute(Subject = as.character(Subject),
#             DVDESC1 = case_when(!str_detect(DVDESC2,"^$|pattern")
#                              ~ str_c(DVDESC1, DVDESC2, sep = " "),
#                              .default = DVDESC1),
#             Preventative = case_when(!str_detect(DVDESC5,"^$|pattern") & !str_detect(DVDESC6,"^$|pattern")
#                              ~ str_c(DVDESC4, DVDESC5, DVDESC6, sep = " "),
#                              .default = DVDESC4)) |> 
  # gt() |>
  # gtsave("pd.docx")
```
...
```{r}
#| label: Import DE
# read_sas(gsub("\\\\", "/", r"(\\wf00168p.oneabbott.com\data1\CDM\ADC-US-RES-23234\OpenClinica\Current\de.sas7bdat)")) |> 
#   filter(!str_detect(DEYN01,regex("Check here if no data recorded",ignore_case = T))) |> 
#   transmute( Subject = Subject,
#             `Sensor SN` = DESPID02,
#              `Date of the Incident` = format.Date(ymd(DEDTC02,tz = "America/Los_Angeles"),format = "%d %b %y"),
#             `Time of the Incident` = DETM01,
...
ps <- ps |>
      ...
      mutate(Subject = as.character(Subject),
             `Ketone Date Time` = ymd_hm(str_c(PSDTC01, PSTM01, sep = " ")),
             KRSEQ01 = PSORES02,
             `Glucose Date Time` = ymd_hm(str_c(PSDTC01, PSTM01, sep = " ")),
             KRSEQ03 = PSORES01,
             `Ref Type` = "Venous strip",
             pump_suspend_start = ymd_hm(str_c(PSDTC01, PSTM02, sep = " ")),
             pump_suspend_end = ymd_hm(str_c(PSDTC01, PSTM03, sep = " "))) |>
      arrange(Subject)
```
...
ps |> 
  mutate(pump_duration = as.numeric(difftime(pump_suspend_end,pump_suspend_start,units = "hours"))) |> 
  summarise(N = n(),
            mean = mean(pump_duration),
            min = min(pump_duration),
            max = max(pump_duration)
            )
```
...
```{r}
kgr12 |> 
  filter(Subject %in% c("90008","1330010")) |> 
  select(Subject, ketone_time = KRDTC02, ketone_reference = KRSEQ01,
                  glucose_time = KRDTC03, glucose_reference = KRSEQ03) |> 
  openxlsx::write.xlsx("ketone&glucose.xlsx")
```
...
ketone_wide <- 
          kgr12 |> 
             ...
             left_join(randox |> 
                         filter(!str_starts(`Sample ID`,"104")) |> 
                          summarise(mean_2uL = mean(`2uL Result`,na.rm = T),
                                    ...
                                    by = join_by(KRSEQ02 == `Sample ID`)) |> 
             left_join(ps |> 
                        select(Subject,pump_suspend_start,pump_suspend_end), 
                        by = join_by(Subject == Subject)) |> 
             mutate(`Ketone Date Time` = case_when(
              ...
              bind_rows(ps |> 
                        select(Subject,`Ketone Date Time`:pump_suspend_end)) |> 
           select(!c(KRDTC02,KRDTC03:IVTM01, `Ref Type`)) |>
...
```{r}
p <- vector("list",length(unique(kgriv121$Subject)))
for (names in unique(kgriv121$Subject)){
p[[names]] <- kgriv121 |> 
  filter(`Ref Type` == "Venous strip") |> 
  # filter(IVVAL01 == "Valid" | is.na(IVVAL01)) |> 
  mutate(kt = as.numeric(difftime(`Ketone Date Time`,first(`Ketone Date Time`),units = "hours")),
         gt = as.numeric(difftime(`Glucose Date Time`,first(`Glucose Date Time`),units = "hours")),
         .by = Subject) |> 
  filter(Subject == names) |> 
  ggplot() + 
  geom_point(aes(x = gt, y = KRSEQ03), color = "#3399ff") + 
  geom_line(aes(x = gt, y = KRSEQ03), color = "#3399ff") +
  geom_point(aes(x = kt, y = KRSEQ01 * 100)) +
  geom_line(aes(x = kt, y = KRSEQ01 * 100)) +
  scale_x_continuous(name = "Time (Hours)", breaks = seq(0, 10, by = 1), limits = c(0,10)) + 
  scale_y_continuous(
    name = "Glucose (mg/dL)", breaks = seq(0, 450, by = 50), limits = c(0,450),
    sec.axis = sec_axis(~ . /100, name = "Ketone (mmol/L)", breaks = seq(0, 4.5, by = 0.5))) +
  theme_bw() +
  labs(title =  names)
   

  ggsave(filename = paste0("plot_", names, ".png"), plot = p[[names]], width = 8, height = 6, dpi = 300)
   # p[[names]] |> 
   #   print()
}
```
<!-- "90006" "1330003" "90008" "1330005" "1330010" -->
...
(kgriv121 |> 
  ...
  mutate(kt = as.numeric(difftime(`Ketone Date Time`,first(`Ketone Date Time`),units = "hours")),
         gt = as.numeric(difftime(`Glucose Date Time`,first(`Glucose Date Time`),units = "hours")),
         .by = Subject) |> 
  ggplot(aes(x = kt, y = KRSEQ01, color = Subject)) + 
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 1,linewidth = 1, linetype = "twodash", color = "red") +
  scale_x_continuous(name = "Time (Hours)", breaks = seq(0, 10, by = 1), limits = c(0,10)) + 
  scale_y_continuous(name = "Ketone (mmol/L)", breaks = seq(0, 4.5, by = 0.5), limits = c(0,4.5)) +
  theme_bw() + 
  ...
  ggsave(filename = "Ketone.png", . ,width = 8, height = 6, dpi = 300)
```
...
(kgriv121 |> 
  ...
  mutate(kt = as.numeric(difftime(`Ketone Date Time`,first(`Ketone Date Time`),units = "hours")),
         gt = as.numeric(difftime(`Glucose Date Time`,first(`Glucose Date Time`),units = "hours")),
         .by = Subject) |> 
  ggplot(aes(x = gt, y = KRSEQ03, color = Subject)) + 
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 225,linewidth = 1, linetype = "twodash", color = "red") +
  scale_x_continuous(name = "Time (Hours)", breaks = seq(0, 10, by = 1), limits = c(0,10)) + 
  scale_y_continuous(name = "Glucose (mg/dL)", breaks = seq(0, 450, by = 50), limits = c(0,450)) +
  theme_bw() + 
  theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),legend.position = "bottom",plot.title = element_text(hjust = 0.5))) %>% 
  ggsave(filename = "Glucose.png", . ,width = 8, height = 6, dpi = 300)
```
...
```{r}
#| label: Left join kgriv12 with Pump suspension

kgriv12 <- kgriv12 |> 
              left_join(ps |> 
                        select(Subject,pump_suspend_start,pump_suspend_end), 
                        by = join_by(Subject == Subject)) 
  # filter(between(`Ketone Date Time`,pump_suspend_start,pump_suspend_end))
```

```{r}
#| eval: false
kgriv12 |> 
     filter(`Ref Type` == "Randox") |> 
     select(Subject, date_time = `Ketone Date Time`, Result = KRSEQ01) |> 
     openxlsx::write.xlsx("Randox.xlsx")
```

```{r}
#| eval: false
vroom("cal_replacement_234_isc_prod1.txt", show_col_types = FALSE) |> 
    transmute(sensor_serial_number = convert_UID_to_sensor_serial_number(sensor_UID),
              slope = slope_na_mm,
              intercept = intercept_na) |> 
    openxlsx::write.xlsx("CalTable.xlsx")
```

```{r}
#| label: Get Time duration Wide
ketone_wide_updated <- 
ketone_wide |> 
          left_join(
            # Find the Peak Time and consider duplicated max values
            ketone_wide |> 
              slice_max(Randox,n = 1,with_ties = F,na_rm = TRUE, by = c(Subject)) |> 
              select(Subject,`Ketone Peak Time` = `Ketone Date Time`),
              by = join_by(Subject)) |> 
          left_join(
            # Find the Peak Time and consider duplicated max values
            ketone_wide |> 
              slice_max(KRSEQ03,n = 1,with_ties = F,na_rm = TRUE, by = c(Subject)) |> 
              select(Subject,`Glucose Peak Time` = `Glucose Date Time`),
              by = join_by(Subject)) |> 
          mutate(`Time < 1 mmol/L` = case_when(max(Randox,na.rm = T) > 1 & 
                                               `Ketone Date Time` > `Ketone Peak Time` & 
                                               Randox < 1  ~ `Ketone Date Time`),
                 ...
                 duration_to_below_1mmol = as.numeric(difftime(`Time < 1 mmol/L`,`Ketone Peak Time` ,units = "hours")),
                 `Time < 250 mg/dL` = case_when(max(KRSEQ03,na.rm = T) > 250 & 
                                                `Glucose Date Time` > `Glucose Peak Time` & 
                                                KRSEQ03 < 250  ~ `Glucose Date Time`),
                 `Time < 250 mg/dL` = first(`Time < 250 mg/dL`,na_rm = T),
                 duration_to_below_250mg = as.numeric(difftime(`Time < 250 mg/dL`,`Glucose Peak Time` ,units = "hours")),
                 .by = c(Subject))
```  
...
ketone <- kgriv12 |> 
          bind_rows(
            ps |> 
              select(Subject,`Ketone Date Time`:pump_suspend_end)) |> 
          arrange(Subject, `Ref Type`, `Ketone Date Time`) |> 
          left_join(
            # Find the Peak Time and consider duplicated max values
            kgriv12 |> 
          ...
          mutate(`Time < 1 mmol/L` = case_when(max(KRSEQ01,na.rm = T) > 1 & 
                 ...
                 duration_to_peak = as.numeric(difftime(`Ketone Date Time`,first(`Ketone Date Time`),units = "hours")),
                 duration_to_below_1mmol = as.numeric(difftime(`Time < 1 mmol/L`,`Peak Time` ,units = "hours")),
                 .by = c(Subject,`Ref Type`))
```
...
ketone |>
   ...
   mutate(across(c(Mean,SD), ~ round(.x + 0.00001,2))) |> 
   gt() |>
...
```{r eval = params$analysis}
#| label: Summary for Ketone Reference Baseline
summary_calculation <- list(Mean = ~ mean(.x, na.rm = T),
                            Median = ~ median(.x, na.rm = T),
                            SD = ~ sd(.x, na.rm = T),
                            Min = ~ min(.x, na.rm = T),
...
ketone |>
   filter(`Ref Type` == "Venous strip") |>
   slice_min(`Glucose Date Time`,by = Subject) |> 
   summarise_at("KRSEQ03",summary_calculation) |>
   # Fix round half to even
   mutate(across(c(Mean,SD), ~ round(.x + 0.00001,2))) |> 
   gt() |>
   cols_align(align = "center") |>
...
```{r eval = params$analysis}
#| label: Summary for Ketone Reference Peak
summary_calculation <- list(Mean = ~ mean(.x, na.rm = T),
                            Median = ~ median(.x, na.rm = T),
                            SD = ~ sd(.x, na.rm = T),
                            Min = ~ min(.x, na.rm = T),
...
```{r eval = params$analysis}
#| label: Summary for Ketone Reference Average
summary_calculation <- list(Mean = ~ mean(.x, na.rm = T),
                            Median = ~ median(.x, na.rm = T),
                            SD = ~ sd(.x, na.rm = T),
                            Min = ~ min(.x, na.rm = T),
...
```{r eval = params$analysis}
#| label: Summary for Ketone Reference End
summary_calculation <- list(Mean = ~ mean(.x, na.rm = T),
                            Median = ~ median(.x, na.rm = T),
                            SD = ~ sd(.x, na.rm = T),
                            Min = ~ min(.x, na.rm = T),
...
ketone |>
   filter(`Ref Type` == "Venous strip") |>
   slice_max(`Glucose Date Time`,by = Subject) |> 
   summarise_at("KRSEQ03",summary_calculation) |>
   # Fix round half to even
   mutate(across(c(Mean,SD), ~ round(.x + 0.00001,2))) |> 
   gt() |>
...
```{r}
ketone_wide_updated |> 
     filter(max(KRSEQ03,na.rm = T) >= 250,.by = c(Subject)) |> 
     slice_max(KRSEQ03, n = 1, with_ties = F,by = c(Subject)) |> 
     summarise(across(c(duration_to_below_250mg),summary_calculation[c(1,3,6)],.names = "{.col} {.fn}")) |>
     gt() 
     # cols_align(align = "center") |>
...
```{r eval = params$analysis}
#| label: Summary for Peak reaching 1 mmol/L
#| column: page
#| tbl-cap: "Summary for Peak reaching 1 mmol/L"
ketone |> 
     filter(max(KRSEQ01,na.rm = T) >= 1,.by = c(Subject,`Ref Type`)) |> 
     slice_max(KRSEQ01, n = 1, with_ties = F,by = c(Subject,`Ref Type`)) |> 
...
#          across(c(`8uL Result`,`2uL Result`), ~ if_else(.x == 0.07,0,.x))
#          # across(c(`8uL Result`,`2uL Result`), ~ replace_na(.x,0))
#          ) |>
#   arrange(`Sample ID`) |> 
  # vroom_write(gsub("\\\\", "/", r"(\\oneabbott.com\dept\ADC\Technical_OPS\Clinical_Affairs\Clinical Study Files\Apollo\ADC-US-RES-23234_IDE Pump Suspension Study\Statistics\Programs\Datasets\AL\randox.csv)"),delim = ",")
...
```{r}
sensor_breaks <- c(seq(0,480,5))

upload_ketone |> 
  semi_join(
        Ap |> 
        filter(Type == "905") |> 
        group_by(`Subject ID`, `Condition ID`, `Sensor Serial Number`,`Ref Type`) |>
        summarise(r = cor(x = KRSEQ01, y = (ANA/100)*1.25, use = "everything", method = "pearson"), .groups = "drop") |> 
        filter(`Ref Type` == "Venous strip") |> 
        slice_max(r, n = 1, with_ties = F, by = `Subject ID`), by = join_by(`Sensor Serial Number` == `Sensor Serial Number`)) |> 
  left_join(ps |>
             select(Subject,pump_suspend_start,pump_suspend_end),
             by = join_by(`Subject ID` == Subject)) |> 
  filter(`Date Time` >= pump_suspend_start - dminutes(6)) |>
  # filter(between(`Date Time`, pump_suspend_start, pump_suspend_end)) |>
  filter(Type == "905") |> 
  mutate(sensor_interval = as.numeric(difftime(`Date Time`,pump_suspend_start,units = "mins"))) |> 
  filter(sensor_interval <= 480) |> 
  mutate(group = cut(sensor_interval, breaks = sensor_breaks,include.lowest = T)) |>
  group_by(`Subject ID`, `Condition ID`, `Sensor Serial Number`, group) |>
  summarise(mean_ana = mean((ANA/100)*1.25, na.rm = T), .groups = "drop") |> 
  pivot_wider(id_cols = c(`Subject ID`:`Sensor Serial Number`), names_from = group, values_from = mean_ana) |> 
  arrange(`Subject ID`) |> 
  openxlsx::write.xlsx("sensor.xlsx")
```

```{r}
ref_breaks <- c(seq(0,480,15))

ketone |> 
  filter(`Ref Type` == "Venous strip") |> 
  mutate(ref_interval = as.numeric(difftime(`Ketone Date Time`,pump_suspend_start,units = "mins")),
         group = cut(ref_interval, breaks = ref_breaks,include.lowest = T)) |>
  filter(ref_interval <= 480) |>
  group_by(Subject, group) |>
  summarise(mean_ketone_ref = mean(KRSEQ01, na.rm = T), .groups = "drop") |> 
  arrange(group) |> 
  pivot_wider(id_cols = c(Subject), names_from = group, values_from = mean_ketone_ref) |> 
  arrange(Subject) |> 
  openxlsx::write.xlsx("ketone.xlsx")
```
...
ketone |> 
  filter(`Ref Type` == "Venous strip") |> 
  mutate(ref_interval = as.numeric(difftime(`Glucose Date Time`,pump_suspend_start,units = "mins")),
         group = cut(ref_interval, breaks = ref_breaks,include.lowest = T)) |>
  filter(ref_interval <= 480) |>
  group_by(Subject, group) |> 
  summarise(mean_glucose_ref = mean(KRSEQ03, na.rm = T), .groups = "drop") |> 
  arrange(group) |> 
  pivot_wider(id_cols = c(Subject), names_from = group, values_from = mean_glucose_ref) |> 
  arrange(Subject) |> 
  openxlsx::write.xlsx("glucose.xlsx")
```

```{r}
#| label: How many patients had a ketone level >1.0 mmol/L 

ketone_wide |> 
  filter(between(`Ketone Date Time`, pump_suspend_start, pump_suspend_end)) |> 
  summarise(N = n_distinct(Subject),
            n_greater1 = n_distinct(Subject[Randox > 1.5]),
            n_greater1less250 = n_distinct(Subject[Randox > 1.5 & KRSEQ03 < 250]))
```

```{r}
ketone_wide |>
  mutate(
    ref_interval = as.numeric(difftime(`Ketone Date Time`, pump_suspend_start, units = "hours")),
    group = cut(ref_interval, breaks = seq(0, 6, 1), include.lowest = TRUE)) |>
  filter(ref_interval <= 6) |>
  group_by(group) |>
  summarise(N = n_distinct(Subject),
            n_greater1 = n_distinct(Subject[Randox >= 1]),
            n_less250 = n_distinct(Subject[KRSEQ03 < 250]),
            n_greater1less250 = n_distinct(Subject[Randox >= 1 & KRSEQ03 < 250]), .groups = "drop") |> 
  mutate(across(c(n_greater1:n_greater1less250), ~ str_c(round_normal(.x / N*100,1), "% (", .x, ")")))
```
...
ketone |>
  ...
  mutate(
    ref_interval = as.numeric(difftime(`Ketone Date Time`, pump_suspend_start, units = "hours")),
    # ref_interval = as.numeric(difftime(pump_suspend_end, pump_suspend_start, units = "hours")),
    group = cut(ref_interval, breaks = seq(0, 6, 1), include.lowest = TRUE)) |>
  filter(ref_interval <= 6) |>
  group_by(group) |>
  summarise(N = n_distinct(Subject),
            n_greater1 = n_distinct(Subject[KRSEQ01 >= 1]),
            n_less250 = n_distinct(Subject[KRSEQ03 < 250]),
            n_greater1less250 = n_distinct(Subject[KRSEQ01 >= 1 & KRSEQ03 < 250]), .groups = "drop") |> 
  mutate(across(c(n_greater1:n_greater1less250), ~ str_c(round_normal(.x / N*100,1), "% (", .x, ")")))
```
...
semi_join(
        ...
        filter(Type == "905") |> 
        group_by(`Subject ID`, `Condition ID`, `Sensor Serial Number`,`Ref Type`) |>
        summarise(r = cor(x = KRSEQ01, y = ANA, use = "everything", method = "pearson"), .groups = "drop") |> 
        filter(`Ref Type` == "Venous strip") |> 
        slice_max(r, n = 1, with_ties = F, by = `Subject ID`), by = join_by(`Sensor Serial Number` == `Sensor Serial Number`)) |> 
  filter(`Ref Type` == "Randox" & Type == "905") |> 
  filter(between(`Ketone Date Time`, pump_suspend_start, pump_suspend_end)) |>
  # filter(`Ketone Date Time` > pump_suspend_end) |>
  ggplot(aes(x = KRSEQ01, y = ANA)) +
  geom_point(alpha = 0.3) + 
  geom_smooth(method = "lm", se = FALSE) + 
  ggpubr::stat_cor(aes(label = ..r.label..)) + 
  # ggpmisc::stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~`,`~")),
  #                       parse = TRUE, coef.digits = 4, rr.digits  = 3) +
  labs(x = "Randox (mmol/L)", y = "Sensor Ketone (mmol/L)" , title = "Sensor Ketone vs Randox") +
...
semi_join(
        ...
        filter(`Ref Type` == "Venous strip") |> 
        slice_max(r, n = 1, with_ties = F, by = `Subject ID`), by = join_by(`Sensor Serial Number` == `Sensor Serial Number`)) |> 
  filter(`Ref Type` == "Venous strip" & Type == "906") |> 
  mutate(sensor_interval = as.numeric(difftime(`Date Time`, pump_suspend_start, units = "hours")),
         ket_interval = as.numeric(difftime(`Ketone Date Time`, pump_suspend_start, units = "hours")),
         gluc_interval = as.numeric(difftime(`Glucose Date Time`, pump_suspend_start, units = "hours"))) |> 
  filter(`Subject ID` == "90004") |> 
  ggplot() +
  # geom_point(aes(x = sensor_interval, y = ANA, color = "Sensor Data")) + 
  geom_line(aes(x = sensor_interval, y = ANA, color = "Sensor Data")) + 
  geom_point(aes(x = ket_interval, y = KRSEQ01 , color = "Blood Ketone")) +
  geom_point(aes(x = gluc_interval, y = KRSEQ03 / 180, color = "Blood Glucose")) +
  scale_y_continuous(name = "Primary Axis Label", sec.axis = sec_axis(~. * 180, name = "Secondary Axis Label"))
```
...
Ap_rate |>
   filter(`Ref Type` == "Randox") |>
   # slice_max(`Glucose Date Time`,by = Subject) |> 
   summarise_at("ref_rate",summary_calculation) |>
   # Fix round half to even
   mutate(across(c(Mean,SD), ~ round(.x + 0.00001,2))) |> 
   gt() |>
   cols_align(align = "center") |>
...
Ap_rate |>
   filter(`Ref Type` == "Randox") |>
   filter(ref_rate > 1) |> 
   summarise(n = n_distinct(`Subject ID`))
```
...
Ap_rate |>
   semi_join(
        ...
        filter(`Ref Type` == "Venous strip") |> 
        slice_max(r, n = 1, with_ties = F, by = `Subject ID`), by = join_by(`Sensor Serial Number` == `Sensor Serial Number`)) |> 
   filter(!is.na(sensor_rate) & !is.na(ref_rate)) |>
   filter(Type == "905") |> 
  filter(`Ref Type` == "Randox") |> 
  # filter(between(`Date Time`, pump_suspend_start, pump_suspend_end)) |> 
  filter(`Date Time` > pump_suspend_end) |> 
  # select(`Subject ID`,`Ref Type`, ref_rate) |> 
  select(`Subject ID`, sensor_rate) |> 
  openxlsx::write.xlsx("sensor_rate.xlsx")
```


```{r eval = params$analysis}
#| label: Histogram of Rate of Change
Ap_rate |>
   semi_join(
        Ap |> 
        filter(Type == "905") |> 
  ...
  filter(`Ref Type` == "Randox") |> 
   ggplot() +
   geom_histogram(aes(x = ref_rate, y = after_stat(density),fill = `Ref Type`),bins = 50) +
   labs(title = "Histogram of Reference Rate") + 
   # facet_grid(cols = vars(`Ref Type`)) + 
...
vs <- vs |>
      ...
      mutate(across(starts_with("VSO"), ~ as.numeric(.x)),
...
ma <- ma |>
      filter(!str_detect(QSYN01,regex("Check here if no data recorded",ignore_case = T))) |>
      mutate(Subject = as.character(Subject)) |>
      arrange(Subject)
```
...
#| eval: false
dm |> 
    select(Subject,SEX:RACE) |> 
    left_join(vs |> 
                 select(Subject,starts_with("VSOR")),
              by = join_by(Subject == Subject)) |> 
    mutate(`Weight(Kg)` = measurements::conv_unit(VSORES03,"lbs","kg"),
          `Height(Inches)` = VSORES01*12 + VSORES02,
          `Height(Meter)` = measurements::conv_unit(`Height(Inches)`,"inch","m"),
           BMI = `Weight(Kg)`/(`Height(Meter)`)**2) |> 
   left_join(mh1 |> 
                select(Subject,MHORES01:last_col()),
              by = join_by(Subject == Subject)) |> 
   left_join(ma |> 
             select(Subject,QSORES01:last_col()),
              by = join_by(Subject == Subject)) |> 
   select(Subject,SEX:RACE, `Weight(Pound)` = VSORES03,`Weight(Kg)`:last_col()) |> View()
   # sjlabelled::label_to_colnames() |> 
   # openxlsx::write.xlsx("Demography.xlsx")
```
...
lb2 <- lb2 |>
       mutate(Subject = as.character(Subject),
              LBORES01 = parse_number(LBORES01)) |>
       filter(!is.na(LBORES01)) |> 
       arrange(Subject)
```
...
lb2 |> 
  summarise(N = n(),
            AIC_7 = n_distinct(Subject[LBORES01 < 7.0]))
```
