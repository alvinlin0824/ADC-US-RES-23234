...
PairKetone <- function(sensor_data){
  ...
  ketone |>
    ...
    slice_min(abs(`Ketone Date Time` - `Date Time`),n = 1,with_ties = F) |>
    ungroup() |> 
    # Select the nearest time
...
Ap <- upload_ketone |> 
      ...
      list_rbind() |> 
      # Rate
      mutate(sensor_rate = ((ANA-lag(ANA))/100) / (as.numeric(difftime(`Date Time`,lag(`Date Time`),units = "hours"))),
      ...
      # Assign Category
          ...
          `Absolute Rate Deviation(mmol/L/hour)` = fct_case_when(
            ...
            `Absolute Rate Deviation` > 3  ~ ">3"),.by = `Sensor Serial Number`) |>
        filter(!is.na(`Rate Deviation`))
```

,`Rate Deviation(mmol/L/hour)`,`Absolute Rate Deviation(mmol/L/hour)`)
      mutate(across(where(is.factor), ~ as.character(.x)))
```{r}
haven::read_sas("ap_rate.sas7bdat") |>
      transmute(`Subject ID` = as.character(subject),
                `Condition ID` = condition_id,
                `Date Time` = with_tz(dtm,tz = "UTC"),
                 `Ketone Date Time` = with_tz(dtm_ref,tz = "UTC"),
                 ANA = ana_100*100,
                 KRSEQ01 = KRSEQ01,
                 `Rate Deviation` = rd
                # `Rate Deviation(mmol/L/hour)` = str_sub(level,3,10),
                # `Absolute Rate Deviation(mmol/L/hour)` = str_sub(level1,3,9)
                 ) |> 
  dplyr::setdiff(
   Ap |>
   select(`Subject ID`,`Condition ID`,`Date Time`,`Ketone Date Time`,ANA,KRSEQ01,`Rate Deviation`)) |> 
...
diff_calculation <- list(Mean = ~ mean(.x, na.rm = T),
                          ...
                          N = ~ sum(!is.na(.x),na.rm = T))
Ap |>
   reframe(across(c(`Rate Deviation`,`Absolute Rate Deviation`),diff_calculation))
   # summarise_at(.vars = c("Rate Deviation","Absolute Rate Deviation"),.funs = diff_calculation)
```
