...
kgriv12 <- kgr12 |> 
           ...
           mutate(`Ketone Date Time` = ymd_hm(str_c(KRDTC01,KRDTC02,sep = " ")),.keep = "unused",.before = KRSEQ03) |> 
...
```{r}
kgriv12 |> 
    filter(max(KRSEQ01) > 1 & `Ketone Date Time` > `Peak Time` & KRSEQ01 < 1) |> View()
```
...
kgriv12 |> 
  left_join(
    ...
    kgriv12 |> 
      ...
      by = join_by(Subject)) |> 
  mutate(`Time < 1 mmol/L` = case_when(max(KRSEQ01) > 1 & `Ketone Date Time` > `Peak Time` &
                                        KRSEQ01 < 1  ~ `Ketone Date Time`)) |> View()
...
kgriv12 |>
   ...
   group_by(Subject) |> 
   # Find the Peak Time and consider duplicated max values
   mutate(ID = KRSEQ01 == max(KRSEQ01)) |> 
   group_by(ID,.add = T) |> 
   mutate(`Peak Time` = case_when(ID == TRUE & row_number() == 1 ~ `Ketone Date Time`),.after = `Ketone Date Time`) |> 
   ungroup(ID) |> 
   fill(`Peak Time`,.direction = "downup") |> 
   # Find the time after peak when Ketone < 1 mmol/L
   mutate(`Time < 1 mmol/L` = case_when(max(KRSEQ01) > 1 & `Ketone Date Time` > `Peak Time` &
                                        KRSEQ01 < 1  ~ `Ketone Date Time`)) |> View()
```
...
kgriv12 |>
  ...
  geom_point() + 
  geom_line() + 
  labs(title = "Ketone Reference",x = "Time(Hrs)", y = "Ketone Test Result (mmol/L)") +
  theme_bw() +
