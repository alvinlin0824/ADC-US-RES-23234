...
kgr12 <- kgr2 |> 
         select(Subject,KRSEQ02:KRSEQ01) |> 
         left_join(kgr1 |> 
...
kgriv12 <- kgr12 |> 
           ...
           bind_rows(
             iv12 |> 
                  left_join(randox |> 
                            summarise(KRSEQ01 = mean(`2uL Result`,na.rm = T)*4,.by = `Sample ID`),
...
#| column: body
ketone |> 
     filter(KRSEQ01 == max(KRSEQ01) & !is.na(`Time < 1 mmol/L`) ,.by = c(Subject,`Ref Type`)) |> 
...
#| column: body
ketone |> 
     filter(KRSEQ01 == max(KRSEQ01),.by = c(Subject,`Ref Type`)) |> 
     slice_min(`Ketone Date Time`, n = 1, with_ties = F,by = c(Subject,`Ref Type`)) |> 
     summarise(across(c(KRSEQ01,duration_to_peak,duration_to_below_1mmol),summary_calculation[c(1,3,6)],.names = "{.col} {.fn}"), .by = c(`Ref Type`)) |> 
     gt() |> 
     cols_align(align = "center") |>
...
ketone |>
  ...
  scale_x_continuous(expand = c(0,0), breaks = c(seq(0,8,1)), limits = c(0,8)) + 
...
ketone |>
  ...
  scale_x_continuous(expand = c(0,0), breaks = c(seq(1,8,1))) + 
  scale_y_continuous(expand = c(0,0), breaks = c(seq(-1.0,1.0,0.2)),limits = c(-1,1)) +
  theme_bw() +
  theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank(),legend.position = "bottom",plot.title = element_text(hjust = 0.5))
```
...
#| fig-align: center
ketone |>
  filter(`Peak Time` <= `Ketone Date Time` & !is.na(duration_to_below_1mmol) >= 1,.by = c(Subject,`Ref Type`)) |> 
...
sys <- Ap |> 
        ...
        mutate(Level = factor(Level,levels = c("< 1 mmol/L",">= 1 mmol/L","Overall")),
               Group = case_when(Level == "< 1 mmol/L" & KRSEQ01 < 1 ~ cut(round(`Absolute Difference(mmol/L)` + 0.0001,1), breaks = ketone_breaks,include.lowest = T),
                             Level == ">= 1 mmol/L" & KRSEQ01 >= 1 ~ cut(round(`Absolute Relative Difference(%)` + 0.001,0), breaks = ketone_breaks_percent,include.lowest = T),
                             Level == "Overall" & KRSEQ01 < 1  ~ cut(round((100*`Absolute Difference(mmol/L)`) + 0.001,0), breaks = ketone_breaks_percent,include.lowest = T),
