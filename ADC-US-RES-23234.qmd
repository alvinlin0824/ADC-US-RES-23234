...
data %>%  
   ...
   mutate(`KM (mmol/L)` =  
    ...
    {if (trans == FALSE) tabyl(.,`KM (mmol/L)`,`Ref (mmol/L)`,`Ref Type`) 
     else tabyl(.,`Ref (mmol/L)`,`KM (mmol/L)`,`Ref Type`)} %>% 
    adorn_totals("col") %>% 
...
Ap |> 
   filter(Site == "RCR") |> 
   concur_fun(trans = F) |> 
   mutate(Site = "RCR",.before = `Ref Type`) |> 
bind_rows(
Ap |> 
   filter(Site == "Yale") |> 
   concur_fun(trans = F) |> 
   mutate(Site = "Yale",.before = `Ref Type`)) |> 
   gt(groupname_col = c("Site","Ref Type")) |> 
   cols_align(align = "center") |>
   sub_missing(columns = everything(),missing_text = "") |>
   opt_stylize(style = 6, color = "blue") |> 
...
Ap |> 
   ...
   tab_header(title = md("Concurrence Analysis by Glucose Level (Ref vs. KM)"))
```
...
#| column: screen
#| eval: false
breakdown_condition_plot(Id = "90005")
```
